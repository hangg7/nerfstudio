...
class Trainer:
    ...
    def __init__(
        ...
        writer.setup_local_writer(
            config.logging,
            max_iter=config.trainer.max_num_iterations,
            banner_messages=banner_messages,
        )
        writer.put_config(
            name="config", config_dict=dataclasses.asdict(config), step=0
        )
    ...
    def setup_optimizers(self) -> Optimizers:
        ...
        camera_optimizer_config = (
            self.config.pipeline.datamanager.camera_optimizer
        )
        param_groups = self.pipeline.get_param_groups()
        if camera_optimizer_config.mode != "off":
            assert camera_optimizer_config.param_group not in optimizer_config
            optimizer_config[
                camera_optimizer_config.param_group + "_train"
            ] = {
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            step = 0
            for step in range(
                self._start_step, self._start_step + num_iterations
            ):
                with TimeWriter(
                ...
                if step_check(
                    ...
                    writer.put_scalar(
                        name="Train Loss", scalar=loss, step=step
                    )
                    writer.put_dict(
                        name="Train Loss Dict",
    ...
    def eval_iteration(self, step):
        ...
        if step_check(step, self.config.trainer.steps_per_eval_batch):
            ...
            writer.put_dict(
                name="Eval Metrics Dict",
                scalar_dict=eval_metrics_dict,
                step=step,
            )
        ...
        if step_check(step, self.config.trainer.steps_per_eval_image):
            with TimeWriter(
                writer, EventName.TEST_RAYS_PER_SEC, write=False
            ) as test_t:
                (
                    metrics_dict,
                    images_dict,
            ...
            for image_name, image in images_dict.items():
                writer.put_image(
                    name=group + "/" + image_name, image=image, step=step
                )
