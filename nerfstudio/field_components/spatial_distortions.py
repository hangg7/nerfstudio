        ...
        if isinstance(positions, Gaussians):
            ...
            contract = lambda x: (
                ...
                - (
                    1
                    / torch.linalg.norm(
                        x, ord=self.order, dim=-1, keepdim=True
                    )
                )
            ) * (
                x / torch.linalg.norm(x, ord=self.order, dim=-1, keepdim=True)
            )
            jc_means = vmap(jacrev(contract))(
                positions.mean.view(-1, positions.mean.shape[-1])
            )
            jc_means = jc_means.view(
                list(positions.mean.shape) + [positions.mean.shape[-1]]
            )
            ...
            cov[mask] = (
                jc_means[mask]
                @ positions.cov[mask]
                @ torch.transpose(jc_means[mask], -2, -1)
            )
